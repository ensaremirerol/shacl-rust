...
fn test_inverse_path() {
    ...
    let results = path.resolve_path_for_given_node(&graph, &bob.into());
...
fn test_one_or_more_path() {
    let graph = setup_test_graph();
    let knows = NamedNodeRef::new("http://example.org/knows").unwrap();
    let path = Path::new().add_element(PathElement::OneOrMore(Box::new(PathElement::Iri(knows))));
...
fn test_zero_or_one_path() {
    let graph = setup_test_graph();
    let knows = NamedNodeRef::new("http://example.org/knows").unwrap();
    let path = Path::new().add_element(PathElement::ZeroOrOne(Box::new(PathElement::Iri(knows))));
...
fn test_complex_path() {
    ...
    let path = Path::new()
        ...
        .add_element(PathElement::Alternative(vec![
            PathElement::ZeroOrMore(Box::new(PathElement::Iri(knows))),
            PathElement::Iri(parent),
        ]));

    let company_x = NamedNodeRef::new("http://example.org/CompanyX").unwrap();
    let results = path.resolve_path_for_given_node(&graph, &company_x.into());
    let bob = NamedNodeRef::new("http://example.org/Bob").unwrap();
