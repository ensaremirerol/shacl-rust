...
/// Every `sh:`-namespace term declared as a constant in this module. Used by
/// the L0004 lint to whitelist known SHACL vocabulary terms; a build error
/// here (typo in a constant name) is preferable to silently dropping a term
/// from the whitelist.
pub fn all_terms() -> &'static [NamedNodeRef<'static>] {
    &[
        SHAPE,
        NODE_SHAPE,
        PROPERTY_SHAPE,
        ...
        INTERSECTION,
        UNION,
        EXPRESSION_CONSTRAINT_COMPONENT,
        EXPRESSION,
        RULE,
        RULE_PROPERTY,
        CONDITION,
...
mod tests {
    ...
    #[test]
    fn all_terms_has_no_duplicates() {
        let terms = all_terms();
        let mut seen = std::collections::HashSet::new();
        for t in terms {
            assert!(seen.insert(t.as_str()), "duplicate term: {}", t.as_str());
        }
    }
}
